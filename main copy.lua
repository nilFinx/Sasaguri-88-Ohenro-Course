...
local function respond(res, type, body)
...
local function escapeHTML(str) return str end
local function escapeHTMLs(str)
	...
	for p, c in utf8.codes(str) do
		...
		buffer[#buffer+1] = replacements[ch] or ch
	end
...
local function onRequest_real(req, res, url)
	...
	if path:find("/api/") then
		...
		if command == "create" then
			...
			if not message or message == "" then
				...
				respond(res, types.json, json.encode(data))
	...
	elseif fs.existsSync("./website"..path) then -- PATH INJECTION? FIXME!
